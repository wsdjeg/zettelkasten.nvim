--=============================================================================
-- run.lua --- test runner for headless Neovim
--=============================================================================
...
local function get_test_files()
  ...
  if pattern:match('^test/') or pattern:match('^test\\') then
    ...
    -- Also try exact match: e.g., "zettelkasten/config_spec.lua"
    if #files == 0 then
      files = vim.split(vim.fn.globpath('test', string.format('%s*_spec.lua', pattern)), '\n')
    end
...
local function run_tests()
  ...
  print('=== Zettelkasten.nvim Test Suite ===')
...
local temp_pattern = '/tmp/zettelkasten_nvim_test_'
